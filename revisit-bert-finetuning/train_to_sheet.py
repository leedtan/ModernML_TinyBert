...
args ={'model_type': 'bert', 'model_name_or_path': 'bert-large-uncased', 'task_name': 'RTE', 
    'do_train':True, 'data_dir':  "../../revisit-bert-finetuning/glue_data", 'max_seq_length': 64, 
    ...
    'output_dir': 'tests/PAPER/BASELINE2', 'all_datasets':True, 
    'reinit_pooler': True, 'normalize': True, 'mixout_layers': 12, 'reinit_layers':0, 'mixout': .3, 
    'trials': 30, 'l2_scaling':0}
...
paramnames = ("RYAN_lay_" + 
							str(lnum[0]) + "_" +
							str(lnum[1]) + "_" +
							str(lnum[2]) + "_" +
							str(lnum[3]) + "_reg_" + 
							str(args.l2_reg_mult) + "_regdecay_" +
							str(args.l2_reg_decay) + "_ep_" + 
							str(args.num_train_epochs) + "_decay_" + 
